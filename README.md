...
5.请求成功后，携带订单号路由跳转，通过query传参

6.获取订单信息，动态展示pay页面

7.按需引入element-ui，使用messageBox并更改内容，展示支付二维码

8.支付成功，路由跳转paySuccess组件

9.弹出框button逻辑

> ### 重点

1.在main.js中引入api获得api对象，将其放在vue原型上，使得所有组件无需引入即可调用各种api

2.生命周期函数不能加async

3.通过请求获取支付状态：需要一直请求来确定支付状态，用定时器。点击遇到问题，弹窗。点击已经支付：判断是否支付成功，如果是则清除定时器，关闭弹窗并路由跳转
...
1.完成center组件，拆分为二级路由：MyOrder、GroupOrder，在routes中完成配置
...
6.学习组件守卫：pay跳转paySuccess     beforeRouteEnter
...
2.MyOrder中数据主要为表格，要考虑合并单元格的情况，以及单元格是否展示（需要复习h5的表格标签）

3.全局前置路由守卫处理未登录的跳转情况：

​	未登录不可跳转到/trade /pay /paySuccess /center/myOrder，先跳转login

​	在login登录后，重新跳转目标页面：在next()中传递query参数，利用路由重定向跳转 next('/login?redirect'+toPath)

​	在login组件中，对登录事件进行判断：如果路由路径中有query参数则跳转query页面，否则跳转/home

4.设置了独享路由守卫之后，如果页面手动刷新，from.path=‘/’。所以放行的条件需要多加个‘/’
...
1.login和register表单验证

2.路由懒加载

3.项目上线，获得dist文件夹，交给后端

> ### 重点

1。表单验证，使用vee-validate（难用，感觉自己封装更方便）

2.在routes中，compument后接()=>import('@/***.vue')